...
const Partners = () => {
  ...
  return (
    <div className='mt-20 relative overflow-hidden bg-black py-12'>
        <div className='flex animate-marquee whitespace-nowrap overflow-x-hidden '>
            {
                duplicatedPartners.map((partner, index) => (
                    <img key={index} src={partner.image} alt={partner.name} className='h-8 mx-6 object-contain min-w-[120px] hover:scale-110 transition-transform duration-200 opacity-50 hover:opacity-100 bg-white px-5 rounded '/>
                ))
            }
            {
                duplicatedPartners.map((partner, index) => (
                    <img key={`duplicate-${index}`} src={partner.image} alt={partner.name} className='h-8 mx-6 object-contain min-w-[120px] hover:scale-110 transition-transform duration-200 opacity-50 hover:opacity-100 bg-white px-5 rounded '/>
                ))
            }
        </div>
    </div>
